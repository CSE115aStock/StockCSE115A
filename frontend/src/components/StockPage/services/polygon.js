...
const alpacaApi = (baseURL = config.BASE_URL) => {
    ...
    const getBars = (symbol,start,timeframe) => api.get('/v2/stocks/'+symbol+'/bars?start='+start+'&timeframe='+timeframe+"&limit=10000")
    const getMultiBars = (symbols,start,timeframe) => api.get('/v2/stocks/bars?symbols='+symbols+'&start='+start+'&timeframe='+timeframe+"&limit=10000")
